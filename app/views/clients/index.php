  ...
  <div class="card-body p-0">
    <table class="table table-hover table-striped mb-0">
      ...
      <tbody>
        <?php if (empty($list)): ?>
          <tr>
            <td colspan="6" class="text-center text-muted py-4">No clients yet. <a href="<?= base_url('?page=clients/create') ?>">Add one</a>.</td>
          </tr>
        <?php else: ?>
          <?php foreach ($list as $row): ?>
            <tr>
              ...
              <td>
                <a href="<?= base_url('?page=clients&id=' . (int)($row['id'] ?? 0)) ?>" class="btn btn-sm btn-outline-primary">View</a>
              </td>
            </tr>
          <?php endforeach; ?>
        <?php endif; ?>
